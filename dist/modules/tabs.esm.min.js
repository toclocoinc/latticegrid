/*!
 * Lattice Grid 1.51.0, tabs module
...
.${NS}{display:flex;flex-direction:column;height:100%;
  font-family:var(--${NS}-font-family,var(--lattice-font-family,system-ui,-apple-system,"Segoe UI",Roboto,Helvetica,Arial,sans-serif));
  font-size:var(--${NS}-font-size,var(--lattice-font-size,13px));
  line-height:var(--${NS}-line-height,var(--lattice-line-height,1.4))}
.${NS}__list{display:flex;flex-wrap:wrap;gap:var(--${NS}-gap,0);background:var(--${NS}-band,#f7f8f9);
  border-bottom:1px solid var(--${NS}-border,#d7dbe0);padding:0;flex:0 0 auto}
.${NS}__tab{font:inherit;appearance:none;display:inline-flex;align-items:center;justify-content:center;
  gap:var(--${NS}-item-gap,6px);min-height:var(--${NS}-min-height,24px);
  border:0;border-radius:0;background:transparent;
  padding:var(--${NS}-pad-y,5px) var(--${NS}-pad-x,12px);cursor:pointer;color:var(--${NS}-muted,#586069)}
.${NS}__tab + .${NS}__tab{border-left:1px solid var(--${NS}-separator,#e7eaee)}
.${NS}__tab:hover{background:var(--${NS}-hover,#ebeef1)}
.${NS}__tab:focus-visible{outline:2px solid var(--${NS}-focus,#2563eb);outline-offset:var(--${NS}-focus-offset,-4px)}
.${NS}__tab[aria-selected="true"]{background:var(--${NS}-bg,#fff);color:var(--${NS}-active,#14171a);font-weight:600;
...
const config=buildConfig(desc,parent&&parent.grid);
const panel=state.panels.get(id);
const grid=state.createGridFn(panel,config);
const entry={grid};
...
tab(id){const e=state.instances.get(id);return e?e.grid:null;},
...
const __entry=__req("packages/modules/tabs/index.js");
export const
createTabs=__entry.createTabs;
