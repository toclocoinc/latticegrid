/*!
 * Lattice Grid 1.53.0, charts module
...
if(!span||!bound)return null;
const shape=CARTESIAN[type];
if(!shape||shape.horizontal||type==='bar'||bound.kind==='category')return null;
const values=[];
for(const value of bound.categories||[]){
const n=bound.kind==='time'?toTime(value):toNumber(value);
if(n!==null&&Number.isFinite(n))values.push(n);
}
if(!values.length)return null;
const{min}=rollingDomain(span,values,now);
let newest=-Infinity;
for(const n of values)if(n>newest)newest=n;
if(newest>=min)return null;
return{span,newest,age:now-newest,points:values.length};
}
function windowedBound(bound,config,type,now){
const span=axisWindowSpan(config);
if(!span||!bound||!Array.isArray(bound.categories))return bound;
const shape=CARTESIAN[type];
if(!shape||shape.horizontal||type==='bar'||bound.kind==='category')return bound;
...
const{min}=rollingDomain(span,(finite),now);
const keep=[];
for(let i=0;i<values.length;i++){
const n=values[i];
if(n===null||!Number.isFinite(n)||n>=min)keep.push(i);
}
...
const __m26=__req("packages/modules/charts/regression.js");
const regressionPlots=__m26["regressionPlots"];
const __m27=__req("packages/modules/charts/registry.js");
const chartTypeDef=__m27["chartTypeDef"];
const registeredChartTypes=__m27["registeredChartTypes"];
const registerChartType=__m27["registerChartType"];
const linearScale=__m4["linearScale"];
const bandScale=__m4["bandScale"];
...
const colourRamp=__m11["colourRamp"];
const path=__m18["path"];
const attr=__m18["attr"];
const __m28=__req("packages/core/src/internal/util.js");
const warnOnce=__m28["warnOnce"];
const drawMeasureAxis=__m1["drawMeasureAxis"];
const drawCategoryAxis=__m1["drawCategoryAxis"];
const drawAxisLines=__m1["drawAxisLines"];
const drawContinuousAxis=__m1["drawContinuousAxis"];
...
Object.defineProperty(__exports,"registerChartType",{enumerable:true,get:function(){return __m27["registerChartType"];}});
Object.defineProperty(__exports,"registeredChartTypes",{enumerable:true,get:function(){return __m27["registeredChartTypes"];}});
...
if(stale){
warnOnce(
`chart:x:window:stale:${this.#spec.x||''}`,
...
...(type==='forest'
?{yLabels:(bound.series[0]&&bound.series[0].points||[]).map((p)=>p.label)}
:{}),
titles:{
left:axis.y.title,
bottom:axis.x.title,
