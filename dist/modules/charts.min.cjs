/*!
 * Lattice Grid 1.39.0, charts module
...
function linearTrend(pairs,forecast){
...
return linearTrend(pairs,spec.forecast);
}
...
function trendText(stats,label,fmt,forecast){
...
if(usable.length>=2){
const shape=['M',usable[0].x,usable[0].upper];
for(let i=1;i<usable.length;i++)shape.push('L',usable[i].x,usable[i].upper);
for(let i=usable.length-1;i>=0;i--)shape.push('L',usable[i].x,usable[i].lower);
shape.push('Z');
...
mean:(v)=>(v.length?v.reduce((t,n)=>t+n,0)/v.length:0),
...
return{col:y.col||null,fn:resolveAggregation(y.fn)};
}
