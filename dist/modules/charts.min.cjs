/*!
 * Lattice Grid 1.63.0, charts module
...
series.points=downsample(indexed,target).map((entry)=>entry.point);
}
}
}
...
const distance=Math.sqrt(dx*dx+dy*dy);
const tolerance=Math.max(3,(0.5+(link.weight||0)*3)/2);
...
const label=`${link.source.label}${ARROW}${link.target.label}`;
return{
heading:label,
rows:[{
label:ctx.bound.measure.title,
value:valueText(link.value,ctx.bound.total||0,ctx.grid.messages),
colour:ctx.scheme.series(link.source.index||0),
}],
datum:{
label,
value:link.value,
...
if(hit){
this.#showTooltip(hit.heading,hit.rows,at);
