/*!
 * Lattice Grid 1.59.0, gantt module
...
#t(key,params){
const supplied=this.#opts.messages;
const grid=this.#controller&&this.#controller.grid;
...
const mappedWrites=isObject(opts.fields)
?['start','end','duration','percentComplete','name'].filter((f)=>opts.fields[f]!==undefined)
:[];
if(mappedWrites.length){
warnOnce(
'gantt-edit-mapped-field',
`[lattice] gantt: this plan maps ${mappedWrites.join(', ')} through \`fields\`, which says how to READ `
+'a task. applyEdit writes the canonical property name, so the edit will not be read back. '
