/*!
 * Lattice Grid 1.69.0, gantt module
...
'gantt.tip.range':'{from} to {to}',
'gantt.tip.duration':'{days}d',
'gantt.tip.slack':'slack {days}d',
});
...
?this.#t('gantt.tip.range',{from:this.#iso(rec.es),to:this.#iso(rec.ef)})
:this.#t('gantt.tip.range',{from:round(rec.es),to:round(rec.ef)});
const parts=[rec.name,dates,this.#t('gantt.tip.duration',{days:round(rec.duration)})];
if(rec.percentComplete!=null)parts.push(`${Math.round(rec.percentComplete)}%`);
parts.push(this.#t('gantt.tip.slack',{days:round(rec.totalFloat)}));
return parts.join(' · ');
}
