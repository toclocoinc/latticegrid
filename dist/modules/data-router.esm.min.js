/*!
 * Lattice Grid 1.31.1, data-router module
...
const shown=new Map();
for(const[k,row]of route.partition)if(passes(route,row))shown.set(k,row);
const add=[];
const update=[];
const remove=[];
...
const sel=link.from.selection;
const keys=sel&&typeof sel.keys==='function'?sel.keys():[];
const src=routeFor(link.from);
const selectedRows=(keys.length&&src)
?keys.map((k)=>src.partition.get(String(k))).filter((r)=>r!==undefined)
:[];
...
routes.push({grid,match,keyOf:resolver(o.rowKey,defaultKeyOf),partition:new Map(),keys:new Map()});
return this;
},
attachDefault(grid,o={}){
fallbackRoute={
grid,match:()=>false,keyOf:resolver(o.rowKey,defaultKeyOf),partition:new Map(),keys:new Map(),isDefault:true,
};
return this;
},
...
links.push(lk);
if(!selSubs.has(source)){
const off=source&&typeof source.on==='function'
?source.on('selection:changed',()=>scheduleFrom(source))
:()=>{};
selSubs.set(source,{off,count:0});
}
selSubs.get(source).count+=1;
recomputeLink(lk);
...
return allRoutes().map((r)=>materialize(r));
},
apply(deltas){
