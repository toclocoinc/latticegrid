/*!
 * Lattice Grid 1.63.2, layout module
...
Object.defineProperty(__exports,"columnOrder",{enumerable:true,get:function(){return columnOrder;}});
...
Object.defineProperty(__exports,"pushRight",{enumerable:true,get:function(){return pushRight;}});
Object.defineProperty(__exports,"pullLeft",{enumerable:true,get:function(){return pullLeft;}});
...
return a.yPos-b.yPos||a.xPos-b.xPos||byId(a,b);
}
function byId(a,b){
const x=String(a.id);
const y=String(b.id);
...
return a.xPos-b.xPos||a.yPos-b.yPos||byId(a,b);
}
...
function pushRight(windows,movedId){
const moved=windows.find((w)=>w.id===movedId);
const settled=moved?[moved]:[];
const rest=windows.filter((w)=>w!==moved).sort(columnOrder);
const guardLimit=windows.length+2;
for(const w of rest){
for(let guard=0;guard<guardLimit;guard++){
const blockers=settled.filter((s)=>overlaps(w,s));
if(blockers.length===0)break;
w.xPos=blockers.reduce((furthest,s)=>Math.max(furthest,s.xPos+s.xSize),w.xPos);
}
settled.push(w);
}
return windows;
}
function pullLeft(windows){
const settled=[];
for(const w of[...windows].sort(columnOrder)){
while(w.xPos>1){
const probe={...w,xPos:w.xPos-1};
if(settled.some((s)=>overlaps(probe,s)))break;
w.xPos-=1;
}
settled.push(w);
...
if(mode==='horizontal'){
pushRight(windows,movedId);
return pullLeft(windows);
}
...
Object.defineProperty(__exports,"setModeState",{enumerable:true,get:function(){return setModeState;}});
...
Object.defineProperty(__exports,"effective",{enumerable:true,get:function(){return effective;}});
Object.defineProperty(__exports,"project",{enumerable:true,get:function(){return project;}});
...
.${NS} .${NS}__chrome{display:flex;align-items:center;gap:6px;flex:0 0 auto;min-width:0;
  ...
  background:var(--${NS}-chrome-bg,var(--${NS}-chrome-bg-auto,#15205A));
...
.${NS} .${NS}__title{flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;
  font-weight:600;
  color:var(--${NS}-chrome-ink,var(--${NS}-title,var(--${NS}-chrome-ink-auto,#FFFFFF)))}
.${NS} .${NS}__grip,.${NS} .${NS}__close,.${NS} .${NS}__mode{font:inherit;appearance:none;border:0;
  background:transparent;
  padding:0 4px;cursor:pointer;flex:0 0 auto;color:var(--${NS}-muted,var(--${NS}-muted-auto,#586069));border-radius:3px}
.${NS} .${NS}__grip{cursor:grab;touch-action:none}
.${NS} .${NS}__window[data-grabbed="true"] .${NS}__grip{cursor:grabbing}
.${NS} .${NS}__grip:hover,.${NS} .${NS}__close:hover,.${NS} .${NS}__mode:hover{
  background:var(--${NS}-hover,#ebeef1)}
.${NS} .${NS}__grip:focus-visible,.${NS} .${NS}__close:focus-visible,.${NS} .${NS}__resize:focus-visible,
.${NS} .${NS}__mode:focus-visible{
  outline:2px solid var(--${NS}-focus,#2563eb);outline-offset:1px}
.${NS} .${NS}__chrome .${NS}__grip,.${NS} .${NS}__chrome .${NS}__close,.${NS} .${NS}__chrome .${NS}__mode{
  color:var(--${NS}-chrome-ink-muted,var(--${NS}-muted,var(--${NS}-chrome-ink-muted-auto,#C9C6EF)))}
.${NS} .${NS}__chrome .${NS}__grip:hover,.${NS} .${NS}__chrome .${NS}__close:hover,
.${NS} .${NS}__chrome .${NS}__mode:hover{
  background:var(--${NS}-chrome-hover,var(--${NS}-hover,var(--${NS}-chrome-hover-auto,#2C3A7A)))}
.${NS} .${NS}__chrome .${NS}__grip:focus-visible,.${NS} .${NS}__chrome .${NS}__close:focus-visible,
.${NS} .${NS}__chrome .${NS}__mode:focus-visible{
  outline-color:var(--${NS}-chrome-focus,var(--${NS}-focus,var(--${NS}-chrome-ink,#FFFFFF)))}
.${NS} .${NS}__window[data-maximised="true"]{z-index:4}
.${NS} .${NS}__window[data-minimised="true"] .${NS}__resize{display:none}
...
.${NS} .${NS}__resize{position:absolute;right:0;bottom:0;width:16px;height:16px;padding:0;
  ...
  color:var(--${NS}-muted,var(--${NS}-muted-auto,#586069))}
...
/* The dark rung. Written into the *-auto names, never into the host-facing
   names, so a host token still outranks the module's own dark values.
   The body-surface ink (muted-auto) is lifted ONLY on the explicit attribute:
   the body's colour is the host's, and only an explicit dark theme says it is
   dark. On the OS-preference arm below it is deliberately absent. */
.${NS}[data-theme="dark"],[data-theme="dark"] .${NS}{
  --${NS}-chrome-bg-auto:#4E5ED6;--${NS}-chrome-ink-auto:#FFFFFF;
  --${NS}-chrome-ink-muted-auto:#EFEEFC;--${NS}-chrome-hover-auto:#3A48B5;
  --${NS}-muted-auto:#7A838C}
@media (prefers-color-scheme:dark){
  .${NS}:not([data-theme="light"]):not([data-theme="light"] *){
    --${NS}-chrome-bg-auto:#4E5ED6;--${NS}-chrome-ink-auto:#FFFFFF;
    --${NS}-chrome-ink-muted-auto:#EFEEFC;--${NS}-chrome-hover-auto:#3A48B5}
}
`;
...
if(state.maximisedId){applyMaximisedGeometry(state);return;}
...
?occupiedRows([...state.windows.values()].map(effective),c.rows)
:c.rows;
...
function applyMaximisedGeometry(state){
const{viewportEl:viewport,canvasEl:canvas}=state;
viewport.style.overflowX='hidden';
viewport.style.overflowY='hidden';
canvas.style.gap=state.config.gap;
canvas.style.gridTemplateColumns='minmax(0, 1fr)';
canvas.style.gridTemplateRows='minmax(0, 1fr)';
...
maximise:{glyph:'\u25a1',restoreGlyph:'\u29c9',key:'layout.maximise'},
minimise:{glyph:'\u2212',restoreGlyph:'\u29c9',key:'layout.minimise'},
});
function makeMode(doc,t,spec,kind){
const button=doc.createElement('button');
button.type='button';
button.className=`${NS}__mode`;
button.setAttribute('data-mode',kind);
setModeState(t,button,spec,false);
return button;
}
function setModeState(t,button,spec,active){
if(!button)return;
const mode=MODES[button.getAttribute('data-mode')]||MODES.maximise;
const title=spec.title||spec.id;
button.textContent=active?mode.restoreGlyph:mode.glyph;
button.setAttribute('aria-label',t(active?'layout.restore':mode.key,{title}));
}
...
if(spec.minimisable){
minimiseEl=makeMode(doc,t,spec,'minimise');
chromeEl.appendChild(minimiseEl);
}
if(spec.maximisable){
maximiseEl=makeMode(doc,t,spec,'maximise');
chromeEl.appendChild(maximiseEl);
}
if(spec.closable){
...
return record.override||record.display||record.spec;
}
function project(state){
const records=[...state.windows.values()];
if(!records.some((r)=>r.minimised)){
for(const r of records)r.display=null;
return false;
}
const shadow=records.map((r)=>({
id:r.spec.id,
xPos:r.spec.xPos,
yPos:r.spec.yPos,
xSize:r.spec.xSize,
ySize:r.minimised?1:r.spec.ySize,
}));
compact(shadow,state.config.compact);
const byId=new Map(shadow.map((entry)=>[entry.id,entry]));
for(const r of records)r.display=byId.get(r.spec.id)||null;
return true;
}
function placeWindow(record){
const spec=effective(record);
...
const setModeState=__m5["setModeState"];
const project=__m5["project"];
const effective=__m5["effective"];
...
const COMPACT_MODES=Object.freeze(['vertical','horizontal','none']);
function compactionMode(value){
if(isNil(value))return'vertical';
if(COMPACT_MODES.includes((value)))return(value);
const shown=typeof value==='string'?JSON.stringify(value):`a ${typeof value}`;
warnOnce('layout.compact.unknown',
`layout: compact: ${shown} is not a compaction mode; expected `
+`${COMPACT_MODES.map((m)=>JSON.stringify(m)).join(', ')}. Falling back to "vertical".`);
return'vertical';
}
...
compact:compactionMode(raw.compact),
...
maximisable:raw.maximisable===true?true:undefined,
minimisable:raw.minimisable===true?true:undefined,
};
}
const CAPABILITIES=Object.freeze(['movable','resizable','closable']);
const DISPLAY_MODES=Object.freeze(['maximisable','minimisable']);
...
maximisable:typeof spec.maximisable==='boolean'?spec.maximisable:undefined,
minimisable:typeof spec.minimisable==='boolean'?spec.minimisable:undefined,
});
const out={
id,
...
for(const mode of DISPLAY_MODES)out[mode]=resolveFlag(own[mode],config[mode]);
...
if(state.maximisedId===spec.id)exitMaximise(state);
...
function showFrame(record,visible){
record.el.style.display=visible?'':'none';
}
function cancelGesture(state,record){
if(state.drag&&(!record||state.drag.record===record)){
const{el}=state.drag.record;
releaseDrag(state);
el.removeAttribute('data-dragging');
hideGhost(state);
}
if(state.grab&&(!record||state.grab.record===record))endGrab(state,state.grab.record);
}
function inMode(state,record){
return state.maximisedId===record.spec.id||record.minimised;
}
function enterMaximise(state,record){
const id=record.spec.id;
if(state.maximisedId===id)return true;
if(state.maximisedId)exitMaximise(state);
if(record.minimised)exitMinimise(state,record);
cancelGesture(state,null);
state.maximisedId=id;
record.override={xPos:1,yPos:1,xSize:1,ySize:1};
record.el.setAttribute('data-maximised','true');
for(const other of state.windows.values())showFrame(other,other===record);
for(const r of state.windows.values())placeWindow(r);
applyGeometry(state);
setModeState(state.t,record.maximiseEl,record.spec,true);
announce(state,state.t('a11y.layout.maximised',{title:record.spec.title||id}));
measureWindows(state);
return true;
}
function exitMaximise(state){
const record=state.windows.get(state.maximisedId);
state.maximisedId=null;
if(!record)return false;
record.override=null;
record.el.removeAttribute('data-maximised');
for(const other of state.windows.values())showFrame(other,true);
for(const r of state.windows.values())placeWindow(r);
applyGeometry(state);
setModeState(state.t,record.maximiseEl,record.spec,false);
const{spec}=record;
announce(state,state.t('a11y.layout.restored',{
title:spec.title||spec.id,x:spec.xPos,y:spec.yPos,
}));
measureWindows(state);
return true;
}
function enterMinimise(state,record){
const{spec}=record;
if(record.minimised)return true;
if(!spec.chrome){
warnOnce(`layout.minimise.chrome.${spec.id}`,
`layout: minimise("${spec.id}") — a window with chrome: false cannot be minimised; `
+'there would be nothing left on screen to restore it with.');
return false;
}
if(state.maximisedId===spec.id)exitMaximise(state);
cancelGesture(state,record);
record.minimised=true;
record.bodyEl.style.display='none';
record.el.setAttribute('data-minimised','true');
reflow(state);
setModeState(state.t,record.minimiseEl,spec,true);
announce(state,state.t('a11y.layout.minimised',{title:spec.title||spec.id}));
settle(state,'minimise');
return true;
}
function exitMinimise(state,record){
if(!record.minimised)return false;
record.minimised=false;
record.bodyEl.style.display='';
record.el.removeAttribute('data-minimised');
reflow(state);
setModeState(state.t,record.minimiseEl,record.spec,false);
announce(state,state.t('a11y.layout.restored',{
title:record.spec.title||record.spec.id,x:record.spec.xPos,y:record.spec.yPos,
}));
settle(state,'restore');
return true;
}
...
function restoreMode(state,record){
if(state.maximisedId===record.spec.id)return exitMaximise(state);
return exitMinimise(state,record);
}
function wireModes(state,record){
if(record.modeOff){record.modeOff();record.modeOff=null;}
const offs=[];
if(record.minimiseEl){
offs.push(listen(record.minimiseEl,'click',()=>{
if(record.minimised)exitMinimise(state,record);
else enterMinimise(state,record);
}));
}
if(record.maximiseEl){
offs.push(listen(record.maximiseEl,'click',()=>{
if(state.maximisedId===record.spec.id)exitMaximise(state);
else enterMaximise(state,record);
}));
}
record.modeOff=offs.length?()=>{for(const off of offs)off();}:null;
}
function wireEscape(state){
state.off.push(listen(state.rootEl,'keydown',(e)=>{
if(state.destroyed||!state.maximisedId)return;
if(!e||e.key!=='Escape'||e.defaultPrevented)return;
const record=state.windows.get(state.maximisedId);
if(e.preventDefault)e.preventDefault();
exitMaximise(state);
if(record&&record.maximiseEl&&isFunction(record.maximiseEl.focus)){
record.maximiseEl.focus();
}
}));
}
...
const record={
spec,...parts,reportedSize:null,closeOff:null,modeOff:null,
override:null,display:null,minimised:false,
};
state.canvasEl.appendChild(record.el);
placeWindow(record);
if(state.maximisedId)showFrame(record,false);
state.windows.set(spec.id,record);
wireClose(state,record);
wireModes(state,record);
state.off.push(()=>{
...
if(inMode(state,record)){
warnOnce(`layout.move.mode.${record.spec.id}`,
`layout: move("${id}") — the window is maximised or minimised and owns its own `
+'geometry; restore() it first.');
return false;
}
const next=boundPlacement(state,{
...
const record=state.windows.get(String(id));
if(!record){
warnOnce('layout.maximise.unknown',`layout: maximise("${id}") — no such window.`);
return false;
}
return enterMaximise(state,record);
},
minimise(id){
const record=state.windows.get(String(id));
if(!record){
warnOnce('layout.minimise.unknown',`layout: minimise("${id}") — no such window.`);
return false;
}
return enterMinimise(state,record);
},
restore(id){
const record=state.windows.get(String(id));
if(!record){
warnOnce('layout.restore.unknown',`layout: restore("${id}") — no such window.`);
return false;
}
return restoreMode(state,record);
},
maximised(){return state.maximisedId;},
minimised(){
return[...state.windows.values()].filter((r)=>r.minimised).map((r)=>r.spec.id);
},
