...
if(ok){
grounded.unshift(c.raw);
continue;
}
flagged.unshift(c.raw);
if(mode==='strip'){
out=out.slice(0,c.start)+UNVERIFIED+out.slice(c.end);
}
}
return{text:out,grounded,flagged};
}
});
...
['stddev','standard deviation',profile.stddev],['outliers','outliers',profile.outliers],
];
...
out.push(fact(`${colId}.top.${String(top.value)}`,
...
for(const f of facts)collectNumbers(f.value,groundedValues);
let filtered=false;
...
facts,
groundedValues,
meta:{kind,filtered,factCount:facts.length},
};
}
});
...
const __m3=__req("packages/modules/ai/redact.js");
const redactor=__m3["redactor"];
const __m4=__req("packages/modules/ai/askdata.js");
const runAskData=__m4["runAskData"];
const applyQuerySpec=__m4["applyQuery"];
const __m5=__req("packages/modules/ai/actor.js");
const runActor=__m5["runActor"];
const applyProposalEdits=__m5["applyProposal"];
const __m6=__req("packages/modules/ai/view.js");
const Emitter=__m6["Emitter"];
const mountPanel=__m6["mountPanel"];
const paintPanel=__m6["paintPanel"];
const explainButton=__m6["explainButton"];
const documentFor=__m6["documentFor"];
const mountAskBar=__m6["mountAskBar"];
const paintAskBar=__m6["paintAskBar"];
const mountActorBar=__m6["mountActorBar"];
const paintActorBar=__m6["paintActorBar"];
