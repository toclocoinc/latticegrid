/*!
 * Lattice Grid 1.53.0, kpi module
...
const fields=gridFields(tiles);
const rows=[];
grid.rows.forEach((r)=>{
if(r&&r.group)return;
const key=r.key;
const obj={__kpiKey:key};
for(const f of fields){
try{obj[f]=grid.rows.value(key,f);}catch{obj[f]=undefined;}
}
rows.push(obj);
});
return rows;
...
for(const t of tiles){
const oldIn=oldRow!=null&&(!t.predicate||!!t.predicate(oldRow));
const newIn=newRow!=null&&(!t.predicate||!!t.predicate(newRow));
...
if(t.predicate)rows=rows.filter((r)=>!!t.predicate(r));
if(t.spark.x){
...
let value;
if(t.isCustom){
const rows=t.predicate?storeVals.filter((r)=>!!t.predicate(r)):storeVals;
value=t.customFn?t.customFn(rows,t):null;
}else{
value=t.acc.value();
}
if(!hasData)value=null;
const status=statusFor(value,t.spec,hasData);
let delta=null;
