/*!
 * Lattice Grid 1.52.0, kpi module
...
style.textContent=`
.${NS}{display:grid;gap:12px;grid-template-columns:repeat(var(--${NS}-cols,auto-fit),minmax(160px,1fr));font:inherit}
...
.${NS}__node{display:flex;align-items:center;gap:8px;padding:5px 8px;border-radius:6px;font:inherit}
.${NS}__node:focus-visible{outline:2px solid var(--${NS}-focus,#2563eb);outline-offset:-2px}
.${NS}__node--branch{cursor:pointer}
.${NS}__node--branch:hover{background:var(--${NS}-hover,#f2f4f7)}
.${NS}__twisty{flex:none;width:1em;text-align:center;font-size:.8em;color:var(--${NS}-muted,#586069)}
...
row.style.setProperty('padding-left',`${8+node.level*16}px`);
}
...
const __m3=__req("packages/modules/kpi/tree.js");
const resolveHierarchy=__m3["resolveHierarchy"];
const decorateNodes=__m3["decorateNodes"];
const branchKeys=__m3["branchKeys"];
const visibleNodes=__m3["visibleNodes"];
const __m4=__req("packages/modules/kpi/messages.js");
const resolveMessages=__m4["resolveMessages"];
