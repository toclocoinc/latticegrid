/*!
 * Lattice Grid 1.62.1, kpi module
...
style.textContent=`
.${NS}{display:grid;gap:12px;grid-template-columns:repeat(var(--${NS}-cols,auto-fit),minmax(160px,1fr));font-family:var(--${NS}-font-family,var(--lattice-font-family,var(--lat-chrome-font-family,system-ui,-apple-system,"Segoe UI",Roboto,Helvetica,Arial,sans-serif)));font-size:var(--${NS}-font-size,var(--lattice-font-size,var(--lat-chrome-font-size,13px)));line-height:var(--${NS}-line-height,var(--lattice-line-height,var(--lat-chrome-line-height,1.4)));--${NS}-meter-w:var(--${NS}-meter-width,calc(34px * var(--lat-chrome-scale,1)))}
.${NS}__tile{border:1px solid var(--${NS}-border,#d7dbe0);border-radius:8px;padding:12px 14px;background:var(--${NS}-bg,#fff);display:flex;flex-direction:column;gap:4px;margin:0}
.${NS}__tile:focus-visible{outline:2px solid var(--${NS}-focus,#2563eb);outline-offset:2px}
.${NS}__label{font-size:.8em;color:var(--${NS}-muted,#586069);order:2}
.${NS}__value{font-size:1.6em;font-weight:600;line-height:1.1;order:1}
...
if(kpi.config.columns)el.style.setProperty(`--${NS}-cols`,String(kpi.config.columns));
const byId=new Map();
...
return`g${KEY_SEP}${(path||[]).map(groupPathElement).join(KEY_SEP)}`;
}
function groupPrefixKeys(path){
const keys=new Array(path.length);
const acc=[];
for(let level=0;level<path.length;level++){
acc.push(groupPathElement(path[level]));
