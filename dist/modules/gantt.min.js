/*!
 * Lattice Grid 1.35.0, gantt module
...
const cell=el(this.#doc,'div',`${SPLIT_NS}__cell${col.kind==='name'?` ${SPLIT_NS}__cell--name`:''}`);
...
const av=el(this.#doc,'div',`${SPLIT_NS}__avatar`);
...
group.appendChild(svg(this.#doc,'rect',{class:`${SPLIT_NS}__bar${crit?` ${SPLIT_NS}__bar--critical`:''}`,x:x0,y:round(barY),width:round(w),height:round(barH),rx:3}));
...
const dir=ex>=sx?1:-1;
const elbow=10;
const d=path(['M',round(sx),round(sy),'H',round(sx+dir*elbow),'V',round(ey),'H',round(ex)]);
const line=svg(this.#doc,'path',{class:`${SPLIT_NS}__link${critical?` ${SPLIT_NS}__link--critical`:''}`,d});
line.setAttribute('data-from',from);
line.setAttribute('data-to',to);
line.setAttribute('data-type',type);
g.appendChild(line);
...
d:path(['M',round(ex),round(ey),'L',round(ex-dir*hh),round(ey-hh),'L',round(ex-dir*hh),round(ey+hh),'Z']),
}));
}
plot.appendChild(g);
}
...
const __m4=__req("packages/modules/gantt/split.js");
const GanttSplitView=__m4["GanttSplitView"];
