/*!
 * Lattice Grid 1.34.0, gantt module
...
function scheduleLeaves(order,byId,edges,projectStart,deadline){
...
return{ok:true,tasks:S,critical,criticalPaths,projectFinish};
}
function computeSchedule(tasks,deps=[],options={}){
const projectStart=asNumber(options.projectStart)??0;
const deadline=asNumber(options.deadline);
...
const leafById=new Map(leafOrder.map((id)=>[id,byId.get(id)]));
const leafEdges=expandDependencies(nd.edges,tree);
const leaf=scheduleLeaves(leafOrder,leafById,leafEdges,projectStart,deadline);
...
projectStart,
projectFinish:leaf.projectFinish,
projectDuration:leaf.projectFinish-projectStart,
};
}
...
const __m3=__req("packages/modules/gantt/schedule.js");
const EPS=__m3["EPS"];
const NS='lat-gantt';
...
function nonWorkingPredicate(spec){
if(spec==='weekends'){
return(day)=>{
const d=fromDayNumber(day);
...
const bounds=[lo,hi];
if(Number.isFinite(o.today))bounds.push(o.today);
...
:Math.max(1,o.width-plotLeft-o.rightPadding);
const totalWidth=zoomPreset?plotLeft+plotWidth+o.rightPadding:o.width;
...
this.#todayX=Number.isFinite(o.today)?round(plotLeft+scale.of(o.today)):null;
...
if(Number.isFinite(o.today))this.#drawToday(plot,x,o.today,o.axisHeight,height);
...
const pred=nonWorkingPredicate(this.#opts.nonWorking);
if(!pred)return;
...
class:`${NS}__shade`,x:x0,y:top,width:round(Math.max(0,x1-x0)),height:height-top,
}));
}
plot.appendChild(g);
}
...
const overdue=Number.isFinite(o.today)&&incomplete&&!rec.isSummary&&rec.ef<o.today;
...
?`${toISODate(rec.es)} – ${toISODate(rec.ef)}`
...
const dir=ex>=sx?1:-1;
const d=path(['M',round(sx),round(sy),
'H',round(sx+dir*elbow),'V',round(ey),'H',round(ex)]);
const line=svg(this.#doc,'path',{class:`${NS}__link${critical?` ${NS}__link--critical`:''}`,d});
...
g.appendChild(line);
g.appendChild(this.#arrowhead(ex,ey,dir,critical));
if(dep.lag){
const label=svg(this.#doc,'text',{
...
const h=4;
const d=path(['M',round(ex),round(ey),
'L',round(ex-dir*h),round(ey-h),
'L',round(ex-dir*h),round(ey+h),'Z']);
return svg(this.#doc,'path',{class:`${NS}__arrowhead${critical?` ${NS}__arrowhead--critical`:''}`,d});
}
...
?`${toISODate(rec.es)} to ${toISODate(rec.ef)}`
...
this.#announce(`${rec.name} moved to ${this.#opts.dateAxis?toISODate(ns):`day ${ns}`}`);
}
if(event.preventDefault)event.preventDefault();
}else if(key==='l'||key==='L'){
if(rec.isSummary)return;
...
const __m2=__req("packages/modules/gantt/time.js");
const toISODate=__m2["toISODate"];
const __m3=__req("packages/modules/gantt/render.js");
const GanttView=__m3["GanttView"];
...
Object.defineProperty(__exports,"toDayNumber",{enumerable:true,get:function(){return __m2["toDayNumber"];}});
Object.defineProperty(__exports,"fromDayNumber",{enumerable:true,get:function(){return __m2["fromDayNumber"];}});
Object.defineProperty(__exports,"toISODate",{enumerable:true,get:function(){return __m2["toISODate"];}});
function emitter(){
const map=new Map();
return{
on(event,fn){
...
let dependencies=Array.isArray(opts.dependencies)?opts.dependencies.map((d)=>({...d})):[];
const projectStart=Number.isFinite(opts.projectStart)?Number(opts.projectStart):0;
const deadline=Number.isFinite(opts.deadline)?Number(opts.deadline):undefined;
...
if(columns)for(const f of['start','end','duration'])if(columns[f])fieldByCol.set(columns[f],f);
let schedule=null;
let destroyed=false;
let view=null;
...
const gridOff=[];
function compute(){
const result=computeSchedule(tasks,dependencies,{projectStart,deadline});
if(result.ok){
schedule=result;
...
dependencies=Array.isArray(next)?next.map((d)=>({...d})):[];
return compute();
},
...
if(patch.start!==undefined)setField(id,'start',patch.start);
if(patch.end!==undefined)setField(id,'end',patch.end);
if(patch.duration!==undefined)setField(id,'duration',patch.duration);
compute();
const changed=[id];
if(autoSchedule&&schedule.ok){
for(const other of schedule.order){
if(other===id)continue;
const rec=schedule.tasks.get(other);
if(rec.isSummary)continue;
if(before.get(other)!==rec.es){
setField(other,'start',rec.es);
changed.push(other);
}
}
if(changed.length>1)compute();
}
if(editOpts.writeBack)pushWrites(changed);
return schedule;
},
compute,
rows:{
