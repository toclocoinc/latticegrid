/*!
 * Lattice Grid 1.39.0, kanban module
...
record(entry){
if(!isObject(entry)||isNil(entry.key))return;
const key=entry.key;
...
this.entries.push({key,from,to,at});
...
const at=this.#now();
keys.forEach((key,i)=>this.log.record({key,from:from[i],to:ev.to,at}));
}));
...
const payload={card,key,field:name,fieldPath,value,origin:'user'};
const allowed=await Promise.resolve(emitter.emitBefore('beforeEdit',payload,'user'));
if(!allowed){
...
const payload={card,key,from,to:toColumn,index:toIndex,origin:'user'};
const decision=emitter.emitBefore('beforeMove',payload,'user');
