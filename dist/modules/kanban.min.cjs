/*!
 * Lattice Grid 1.34.0, kanban module
...
if(isFunction(state.filterFn)&&!state.filterFn(card.row,card))return false;
...
const order=configured.map((l)=>l.id);
...
doneColumns:new Set([
...(Array.isArray(cfg.doneColumns)?cfg.doneColumns.map(String):[]),
...(Array.isArray(cfg.columns)
?cfg.columns.filter((c)=>isObject(c)&&c.done).map((c)=>String(c.id)):[]),
]),
...
function cardLabel(card,column,index,total){
const title=card.fields.title||String(card.key);
return`${title}, ${column.title}, ${index+1} of ${total}`;
}
function renderCard(ctx,card,column,index,isReadonly,doc){
...
el.setAttribute('aria-label',cardLabel(card,column,index,column.count));
...
list.setAttribute('aria-label',`${column.title}, ${column.count} card${column.count===1?'':'s'}`);
...
const add=(card,i)=>{
const cardEl=renderCard(ctx,card,column,i,board.isReadonly,doc);
...
const{start,end,padTop,padBottom}=windowRange(list.scrollTop||0,viewport,v.rowHeight,cards.length,v.overscan);
list.appendChild(spacer(doc,padTop));
for(let i=start;i<end;i++)add(cards[i],i);
list.appendChild(spacer(doc,padBottom));
if(!list.__virtualWired){
list.addEventListener('scroll',()=>fillList(board,list,cards,column,laneId));
...
list.setAttribute('role','list');
list.setAttribute('aria-label',`${lane.title}, ${cell.title}, ${cell.count} card${cell.count===1?'':'s'}`);
...
if(board.colDrag){if(ev.preventDefault)ev.preventDefault();return;}
...
const renderBoard=__m2["renderBoard"];
...
'card:move','card:reverted','selection:changed','column:collapse','card:add',
'drag:start','drag:end','swimlane:collapse','column:reorder','filter:changed',
'sprint:changed','epic:changed','card:expand','card:drill','card:edit',
]);
...
try{obj[prop]=grid.rows.value(key,prop);}catch{obj[prop]=undefined;}
}
rows.push(obj);
});
return rows;
...
filterFn:isFunction(cfg.filter)?cfg.filter:null,
...
if(board.el)renderBoard(board);
return board;
};
board.refresh=()=>{seed();return regroup();};
...
board.setFilter=(fn)=>{
state.filterFn=isFunction(fn)?fn:null;
board.fire('filter:changed',{quickFilter:state.quickFilter,hasFilter:!!state.filterFn});
return regroup();
};
...
board.sprints=()=>(ctx.sprintProperty?board.facets(ctx.sprintProperty).map((f)=>f.value):[]);
...
board.applyEdit=async(key,name,value)=>{
const card=board.model.cardsByKey.get(key);
const spec=ctx.cardSpecs[name];
if(!card||!spec||typeof spec.field!=='string')return false;
const fieldPath=spec.field;
const payload={card,key,field:name,fieldPath,value};
if(gridBound){
cfg.grid.edit.setCells([{key,colId:fieldPath,value}],'cell');
board.refresh();
...
if(isNil(key)){
key=uid('card');
if(typeof board.rowKey==='string')setPath(row,board.rowKey,key);
}
...
const vetoFilter=async(keys,toColumn,toIndex)=>{
const accepted=[];
...
const r=cfg.onBeforeMove(card,from,toColumn,toIndex);
ok=(r instanceof Promise)?await r:r;
}
if(ok!==false)accepted.push({key,card,from});
}
return accepted;
};
board.move=async(keys,toColumn,toIndex=null,toLane=undefined)=>{
const keyArr=(Array.isArray(keys)?keys:[keys]).filter((k)=>board.model.cardsByKey.has(k));
const accepted=await vetoFilter(keyArr,toColumn,toIndex);
...
writes.push({key,colId:ctx.columnProperty,value:toColumn});
if(orders&&ctx.orderProperty)writes.push({key,colId:ctx.orderProperty,value:orders[i]});
if(writeLane)writes.push({key,colId:ctx.swimlaneProperty,value:toLane});
});
grid.edit.setCells(writes,'cell');
board.refresh();
...
for(const row of change.update||[])rowStore.set(ctx.key(row),row);
