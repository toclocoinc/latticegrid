/*!
 * Lattice Grid 1.68.0, data-router module
...
__def("packages/core/src/model/rowkey.js",function(__exports,__req){
'use strict';
Object.defineProperty(__exports,"KEY_DEPENDENT_FEATURES",{enumerable:true,get:function(){return KEY_DEPENDENT_FEATURES;}});
Object.defineProperty(__exports,"compositeKey",{enumerable:true,get:function(){return compositeKey;}});
Object.defineProperty(__exports,"createRowKey",{enumerable:true,get:function(){return createRowKey;}});
Object.defineProperty(__exports,"warnMissingKeys",{enumerable:true,get:function(){return warnMissingKeys;}});
Object.defineProperty(__exports,"groupPathKey",{enumerable:true,get:function(){return groupPathKey;}});
Object.defineProperty(__exports,"splitPathKey",{enumerable:true,get:function(){return splitPathKey;}});
const __m0=__req("packages/core/src/internal/util.js");
const warnOnce=__m0["warnOnce"];
const pathGetter=__m0["pathGetter"];
const isFunction=__m0["isFunction"];
const isBlank=__m0["isBlank"];
const KEY_DEPENDENT_FEATURES=[
'changes (rows.apply / rows.queue)',
'streaming dedupe',
...
if(t==='number'||t==='bigint'||t==='boolean')return String(value);
...
function createRowKey(spec,opts={}){
...
try{
const value=fn(data);
return Array.isArray(value)?compositeKey(value):part(value);
}catch(err){
warnOnce('rowKey.throw','rowKey function threw; falling back to index identity for the affected rows.',err);
return`#${index??-1}`;
}
},
keyEmpty(data){
try{
const value=fn(data);
return Array.isArray(value)?value.length===0||value.some(isBlank):isBlank(value);
}catch{
return false;
}
},
missingFields(){return[];},
};
}
if(Array.isArray(spec)&&spec.length>0){
const fields=spec.filter((f)=>typeof f==='string');
const getters=fields.map((f)=>pathGetter(f));
...
for(let i=0;i<n;i++)parts[i]=getters[i](data);
return compositeKey(parts);
},
keyEmpty(data){
for(let i=0;i<n;i++)if(isBlank(getters[i](data)))return true;
return false;
},
missingFields(data){
const out=[];
for(let i=0;i<n;i++)if(isBlank(getters[i](data)))out.push(fields[i]);
return out;
},
};
}
if(typeof spec==='string'&&spec.length>0){
const get=pathGetter(spec);
return{
stable:true,
fields:[spec],
disabled:[],
...
keyEmpty(data){
return isBlank(get(data));
},
missingFields(data){
return isBlank(get(data))?[spec]:[];
},
};
}
if(warn){
warnOnce(
'rowKey.missing',
`no rowKey configured, so rows are identified by object identity. This disables: ${KEY_DEPENDENT_FEATURES.join(', ')}. Set rowKey to a field name, an array of field names, or a function.`,
);
}
return{
stable:false,
fields:[],
disabled:KEY_DEPENDENT_FEATURES.slice(),
key(data,index){
if(data===null||typeof data!=='object'){
return`#${index??0}:${String(data)}`;
}
let assigned=autoKeys.get(data);
if(assigned===undefined){
assigned=`#${autoKeySeq++}`;
autoKeys.set(data,assigned);
}
return assigned;
},
};
...
let autoKeySeq=0;
function warnMissingKeys(resolver,rows){
if(!resolver||resolver.stable===false||!isFunction(resolver.keyEmpty))return;
...
const data=rows[i];
if(!resolver.keyEmpty(data))continue;
missing++;
if(sampleFields===null){
sampleFields=isFunction(resolver.missingFields)?resolver.missingFields(data):[];
sampleData=data;
}
}
if(missing===0)return;
const dedupFields=resolver.fields.length?resolver.fields.join(','):'(function)';
const named=sampleFields&&sampleFields.length
?sampleFields.join(', ')
:(resolver.fields.length?resolver.fields.join(', '):'the rowKey function');
const present=sampleData!==null&&typeof sampleData==='object'
?(Object.keys((sampleData)).join(', ')||'(no fields)')
:String(sampleData);
warnOnce(
`rowKey.emptyValue:${dedupFields}`,
`rowKey ${named} is missing on ${missing} of ${rows.length} rows; the first such row has: ${present}. `
+'Rows without a key value collapse onto one key, so selection, edits and rows.apply updates land on '
+'whichever row now shares it. Set rowKey to a field present on every row, or fix the source data.',
);
}
function groupPathKey(path){
return compositeKey(path);
}
...
const __m2=__req("packages/core/src/model/rowkey.js");
const compositeKey=__m2["compositeKey"];
const __m3=__req("packages/modules/data-router/devtools.js");
const mountRouterDevtools=__m3["mountRouterDevtools"];
...
let routeSeq=0;
function nextRouteId(){routeSeq+=1;return`r${routeSeq}`;}
...
return{key:compositeKey(vals),fields};
};
}
...
const result=route.sink(change);
warnRejectedRemovals(route,result);
}
return{added:change.add.length,updated:change.update.length,removed:change.remove.length};
};
const warnRejectedRemovals=(route,result)=>{
const rejected=(result)?.rejected;
if(!Array.isArray(rejected)||!rejected.length)return;
const removals=rejected.filter((r)=>r&&r.operation==='remove');
if(!removals.length)return;
const name=route.label||`route ${route.routeId}`;
warnOnce(
`data-router:remove-rejected:${route.routeId}`,
`route "${name}" asked its target to remove ${removals.length} row(s) the target does not have`
+` (for example ${JSON.stringify(String(removals[0].id))}). The rows are still on screen.`
+' This is a key mismatch: the key the route derives for a row is not the key the grid derives'
+' for it. Check that the route\'s `rowKey` names the same value as the grid\'s.',
);
};
...
shown.set(compositeKey([route.keyOf(row)]),route.transform?route.transform(row):row);
}
}
const add=[];
const update=[];
