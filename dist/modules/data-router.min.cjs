/*!
 * Lattice Grid 1.34.0, data-router module
...
const needsAll=full||!!(residual.filters||residual.sort||residual.quick)||!caps.range;
const pushed={
...request,
...
return{pushed,residual,needsAll,unpushed,full};
}
...
default:
warnOnce(
`data-router:where-op:${cond.op}`,
`query() cannot finish the residual operator "${cond.op}" client-side, so rows are not narrowed by it; push it to an adapter that supports it, or use a supported operator.`,
);
return true;
}
}
...
apply(deltas){router.apply(normalizeDeltas(source,deltas));return this;},
push(delta){router.push(normalizeDeltas(source,Array.isArray(delta)?delta:[delta]));return this;},
remove(){
const deltas=[];
...
?predicate
:(row)=>partOf(row)===predicate;
routes.push(makeRoute(handler,match,o));
return this;
},
