/*!
 * Lattice Grid 1.9.0 — charts module
...
const stackable=type==='bar'||type==='area'||type==='horizontalBar';
const percent=ctx.stack==='percent'&&stackable&&visible.length>1;
const stacked=(percent||!!ctx.stack)&&visible.length>1&&stackable;
const horizontal=!!shape.horizontal;
const spread=type==='waterfall'
?runningExtent(visible[0])
:measureValues(visible,stacked,percent);
...
?(stacked?xScale.bandwidth:xScale.bandwidth/Math.max(1,visible.length))
:0;
...
drawPoints({
pool,series,xScale,yScale,colour,index:series.index,
...
const seriesId=spec.series||null;
...
let source;
if(spec.from==='groups')source=topGroups(grid);
...
const limited=typeof spec.limit==='number'&&spec.limit>0
...
kind:scaleKindFor(dimensionColumn,seenValues),
...
if(limits){
rule(limits.centre,`${NS}__control-centre`);
rule(limits.upper,`${NS}__control-limit`);
rule(limits.lower,`${NS}__control-limit`);
}
if(capability){
if(capability.lower!==null)rule(capability.lower,`${NS}__spec-limit`);
if(capability.upper!==null)rule(capability.upper,`${NS}__spec-limit`);
if(capability.target!==null)rule(capability.target,`${NS}__spec-target`);
}
rules.finish();
const broken=new Set((capability&&capability.violations?capability.violations:[])
.map((v)=>v.index));
const pool=new Pool(ctx.groups.marks);
const d=[];
const points=[];
...
d.push(i?'L':'M',x,y);
points.push({x,y,value:values[i],violating:broken.has(i)});
}
...
return{xScale,yScale,rightScale,series:visible,bound};
}
...
NA:[[-168,65],[-140,70],[-95,82],[-60,82],[-52,60],[-65,45],[-80,25],
[-97,16],[-105,20],[-115,30],[-125,40],[-140,60],[-168,65]],
SA:[[-81,12],[-60,12],[-50,0],[-35,-8],[-38,-22],[-48,-34],[-58,-40],
[-65,-55],[-75,-50],[-72,-30],[-70,-18],[-81,0],[-81,12]],
EU:[[-10,36],[-9,44],[-5,52],[2,58],[5,62],[15,70],[30,71],[40,66],
[55,60],[50,50],[40,45],[28,41],[20,40],[10,37],[-10,36]],
AF:[[-17,21],[-16,12],[-8,5],[8,4],[10,-2],[12,-12],[14,-22],[20,-34],
[32,-26],[40,-15],[51,-2],[43,11],[35,22],[25,32],[10,37],[-6,36],
[-17,21]],
AS:[[26,40],[45,40],[60,55],[80,72],[110,75],[140,72],[170,66],[160,55],
[140,45],[130,34],[122,22],[105,10],[95,5],[80,8],[72,20],[60,25],
[45,12],[35,30],[26,40]],
OC:[[113,-22],[130,-12],[142,-11],[153,-25],[150,-38],[140,-38],[130,-32],
[115,-34],[113,-22]],
AN:[[-180,-70],[-120,-74],[-60,-78],[0,-70],[60,-68],[120,-70],[180,-72],
[180,-85],[-180,-85],[-180,-70]],
});
...
const pool=new Pool(ctx.groups.marks);
const regions=[];
const codes=useCountries?[...supplied.keys()]:CONTINENTS;
...
if(given===true)return{mode:'filter'};
if(given==='filter'||given==='zoom'||given==='select')return{mode:given};
return{mode:null};
}
...
return{kind:'set',values,range:null};
}
const from=xScale.invert(low);
const to=xScale.invert(high);
return{kind:'range',values:[],range:[from,to]};
}
...
#brush(from,to){
const mode=resolveBrush(this.#spec).mode;
if(!mode||!this.#drawn||!this.#drawn.xScale||!this.#bound)return;
const result=brushed({
bound:this.#bound,xScale:this.#drawn.xScale,from,to,
});
...
axis:{
...(this.#spec.axis||{}),
x:{...((this.#spec.axis||{}).x||{}),min:result.range[0],max:result.range[1]},
},
};
this.draw();
}
...
...shared,pareto:type==='pareto',reference:this.#spec.reference,
});
