/*!
 * Lattice Grid 1.51.0, kpi module
...
clear(el);
for(const t of kpi.model.tiles)el.appendChild(tileNode(doc,t));
if(!kpi.wired)wire(kpi);
}
function tileNode(doc,t){
...
const spoken=t.status===UNKNOWN_STATUS?EMPTY_TEXT:t.status;
fig.setAttribute('aria-label',`${t.label}: ${t.formatted}${spoken?`, ${spoken}`:''}`);
const value=doc.createElement('div');
value.classList.add(`${NS}__value`);
value.textContent=t.formatted;
...
empty.classList.add(`${NS}__empty`);
empty.textContent=EMPTY_TEXT;
fig.appendChild(empty);
}
if(t.delta!==null&&t.delta!==undefined)fig.appendChild(deltaNode(doc,t));
const spark=sparkNode(doc,t);
if(spark)fig.appendChild(spark);
return fig;
}
function deltaNode(doc,t){
...
div.classList.add(`${NS}__delta`,`${NS}__delta--${dir}`);
div.textContent=`${arrow} ${t.deltaFormatted??t.delta}${pct}`;
return div;
}
function sparkNode(doc,t){
...
'tile:click','tile:dblclick','tile:contextmenu','change',
]);
...
kpi.getState=()=>({rows:[...rowStore.values()]});
